...
def welcome_hero():
    ...
    prompts = [
        ("triage: 220-unit MF in Austin, $62M ask, 4.9% cap", "deal_triage"),
        ("pf: 5-year pro forma for Vista East Austin at 4% rent growth", "pro_forma_builder"),
        ("abstract: force majeure across my industrial leases", "lease_abstractor"),
        ("memo: draft the investment memo for Arden Buckhead", "investor_memo"),
        ("rentopt: where is in-place most below market?", "rent_optimization"),
        ("comps: multifamily Austin Class A 2020+ vintage", "comp_finder"),
    ]
    return Div(
        Div(
            Span("◆", cls="hero-mark"),
            H1("Bricksmith", cls="welcome-title"),
            P("Your CRE deal AI squad. Type a prompt — the router picks the right specialist.",
              cls="welcome-sub"),
            cls="welcome-head",
        ),
        Div(
...
def left_pane(*, user_email: str | None, sessions: list[dict], current_sid: str = "",
    ...
    return Div(
        Div(
            A(Span("◆", cls="brand-mark"), Span("Bricksmith"),
              href="/", cls="brand-link"),
    ...
    return Div(
        Div(
            Div(
                ...
                Span("Bricksmith", cls="chat-header-title"),
    ...
    return Div(
        ...
        Div(
            Div(
                ...
                P("Artifacts appear here as agents produce them — property sheets, pro-formas, comps tables, memo previews, RAG citations.",
