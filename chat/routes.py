...
def _ensure_user(sess) -> tuple[int | None, str | None]:
    ...
    with connect() as conn, conn.cursor() as cur:
        cur.execute(
            "INSERT INTO bricksmith.users (email) VALUES (%s) "
...
def _ensure_session(user_id: int, sid: str | int | None, first_message: str | None = None) -> int:
    if sid:
        try:
            ...
            sid_int = 0
        if sid_int:
            row = fetch_one("SELECT id FROM bricksmith.chat_sessions WHERE id = %s AND user_id = %s",
                            (sid_int, user_id))
            if row:
                return sid_int
    ...
    with connect() as conn, conn.cursor() as cur:
        cur.execute(
            "INSERT INTO bricksmith.chat_sessions (user_id, title) VALUES (%s, %s) RETURNING id",
            (user_id, title),
        )
...
def _list_sessions(user_id: int, limit: int = 30) -> list[dict]:
    return fetch_all(
        ...
        "FROM bricksmith.chat_sessions WHERE user_id = %s "
...
def _session_messages(session_id: int) -> list[dict]:
    return fetch_all(
        "SELECT role, content, agent_slug FROM bricksmith.chat_messages "
...
def _persist_message(session_id: int, role: str, content: str,
    ...
    with connect() as conn, conn.cursor() as cur:
        cur.execute(
            "INSERT INTO bricksmith.chat_messages (session_id, role, content, agent_slug, tool_calls) "
            "VALUES (%s, %s, %s, %s, %s::jsonb)",
            (session_id, role, content, agent_slug,
             json.dumps(tool_calls) if tool_calls else None),
        )
        cur.execute(
            "UPDATE bricksmith.chat_sessions SET updated_at = now() WHERE id = %s",
            (session_id,),
        )
        conn.commit()
...
def app_home(sess, sid: str = ""):
    ...
    if uid and sid:
        try:
            sid_int = int(sid)
            row = fetch_one("SELECT id, agent_slug FROM bricksmith.chat_sessions "
...
async def chat_stream(request: Request):
    ...
    if not uid:
        ...
        with connect() as conn, conn.cursor() as cur:
            cur.execute(
                ...
                (f"guest+{id(sess):x}@bricksmith.local",),
            )
            uid = cur.fetchone()[0]
            conn.commit()
        set_user_id(sess, uid)
    ...
    async def event_stream():
        ...
        with connect() as conn, conn.cursor() as cur:
            cur.execute("UPDATE bricksmith.chat_sessions SET agent_slug = %s WHERE id = %s",
