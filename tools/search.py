...
Used by the four sourcing agents when they need information beyond the
synthetic CRE corpus (recent market news, macro signals, named-property
context). Hidden behind one StructuredTool. If neither API key is set we
return a neutral "search unavailable" string so agents degrade gracefully
rather than crash.
"""
...
def _web_search(**kw) -> str:
    ...
    if not data:
        return ("Search unavailable — no TAVILY_API_KEY / EXA_API_KEY configured, "
                "or both providers failed.")
...
web_search = StructuredTool.from_function(
    func=_web_search,
    name="web_search",
    description=("Search the public web for CRE market news, recent comparable sales, "
                 "broker listings, or anything outside the Bricksmith synthetic corpus. "
                 "Returns title + URL + snippet. Tavily is preferred; EXA is fallback."),
