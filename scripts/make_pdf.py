"""Generate a Bricksmith product tour PDF from the demo screenshots.
...
def _footer(canvas, doc):
    ...
    canvas.drawString(2 * cm, 1.2 * cm,
                      "Bricksmith — agentic AI for commercial real estate (synthetic demo data)")
...
def build() -> None:
    ...
    # ── title page ───────────────────────────────────────────────
    story += [
        Paragraph("Bricksmith", styles["title"]),
        Paragraph(
        ...
        Paragraph(
            "Bricksmith is a full agentic system — not a prompt pack. 22 LangGraph ReAct "
    ...
    sections = [
        ...
        (
            ...
            "cap — persisting the run in `bricksmith.pro_formas` — and prints the "
